  ...
  >
    ...
    >
      ...
      <div class="flex flex-col pb-2 px-6 leading-normal w-full">
        ...
        <div v-if="resource === 'events'" class="flex flex-col just text-xs mb-2">
          <div class="w-full flex">
            <p class>{{ moment(card.start_date).format('MMMM DD, YYYY') }}</p>

            <p v-if="card.starts_at">&nbsp;at&nbsp;{{ moment(card.starts_at).format("HH:mm") }}</p>

            <span v-if="card.ends_at">
              <p class>&nbsp;-&nbsp;{{ moment(card.ends_at).format('h:mm') }}&nbsp;</p>
            </span>
          </div>

          <p
            class
            v-if="card.timezone && card.timezone.timezone_value"
          >{{ card.timezone.timezone_value.name[language] }}</p>
        </div>
