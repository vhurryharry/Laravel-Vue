<template>
  <form class="flex-col pt-6 pb-2 my-2">
    ...
    >{{ $t('shared.are_you_sure_you_want_to_leave') }} {{ community.name }}</p>
...
export default {
  ...
  methods: {
    async leave() {
      ...
      await axios
        .post("/community/" + this.community.slug + "/leave", {
          community: this.community
        })
        .then(response => {
          this.$store.dispatch("ui/setActiveModal", null);

          if (this.community.privacy.privacy_value.key === "secret") {
          ...
          this.$toast.open({
            duration: 5000,
            message: this.$t("components.communitie.toast.community_left"),
            position: "is-bottom",
            type: "is-success"
          });
        })
